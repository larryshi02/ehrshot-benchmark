...
class MultiTaskBackwardsReasoningPipeline:
    ...
    def _process_task_sequential(
        ...
        for index, example in enumerate(examples):
            ...
            if example.total_tokens:
                ...
                logger.info(
                    f"[{task_name}] Example {index + 1} used {example.total_tokens} tokens "
    ...
    def _process_task_parallel(
        ...
        def process_single_example(example: SFTExample, index: int) -> Tuple[SFTExample, int]:
            ...
            processed_example = generator.generate_backwards_reasoning(example)
            
            if processed_example.total_tokens:
                logger.info(
                    f"[{task_name}] Example {index + 1} (Patient {example.patient_id}) "
                    f"used {processed_example.total_tokens} tokens"
                )
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(future_to_index):
                try:
                    ...
                    if processed_example.total_tokens:
                        with tokens_lock:
                            total_tokens += processed_example.total_tokens
...
def main() -> None:
    ...
    for task_name in args.tasks:
        task_output_file = build_output_path(args.output_dir, task_name, "backwards_reasoning")
        task_sft_file = build_output_path(args.sft_dataset_dir, task_name, "sft_dataset")
