#!/usr/bin/env python3
"""
Azure-based balanced reasoning pipeline for supervised fine-tuning across multiple tasks.

Key features:
  - For each POSITIVE label in the TRAINING set, generates additional GPT-5 reasoning traces
    to balance the dataset (e.g., if 10% positive, generates 8 additional traces per positive)
  ...
  - Does NOT generate additional traces for negative training samples
"""
...
class BalancedMultiTaskBackwardsReasoningPipeline:
    ...
    def process_task(
        ...
        *,
        ...
        """
        Generate balanced backwards reasoning traces for a single task.
        """
        logger.info(f"=== Processing task: {task_name} ===")

        # Load training examples
        train_examples = self._load_examples_for_task(task_name, TRAIN_SPLIT_NAME)
        train_positive = [e for e in train_examples if e.label_value]
        train_negative = [e for e in train_examples if not e.label_value]
        
        logger.info(
            f"[{task_name}] Training set: {len(train_positive)} positive, {len(train_negative)} negative"
        )

        # Calculate how many additional traces to generate per positive example
        additional_traces_per_positive = calculate_additional_traces_needed(
            len(train_positive), len(train_negative)
        )
        logger.info(
            f"[{task_name}] Will generate {additional_traces_per_positive} additional traces "
            f"per positive training example to balance dataset"
        )
        ...
        # Process training examples (only positive, multiple traces each)
        train_stats = self._process_training_positive(
            task_name,
            train_positive,
            additional_traces_per_positive,
            train_output_file,
        ...
        return TaskProcessingStats(
            task_name=task_name,
            num_train_examples=len(train_positive) * (1 + additional_traces_per_positive),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--train_output_dir",
        type=str,
        help="Directory to save training backwards reasoning traces (per-task JSON files).",
    )
    parser.add_argument(
        "--train_sft_dataset_dir",
        type=str,
        help="Directory to save training SFT dataset entries (per-task JSON files).",
    )
    parser.add_argument(
        "--val_output_dir",
        type=str,
...
def main() -> None:
    ...
    for task_name in args.tasks:
        train_output_file = build_output_path(
            args.train_output_dir, task_name, "balanced_backwards_reasoning"
        )
        train_sft_file = build_output_path(
            args.train_sft_dataset_dir, task_name, "balanced_sft_dataset"
        )
        val_output_file = build_output_path(
            args.val_output_dir, task_name, "balanced_backwards_reasoning"
        )
        val_sft_file = build_output_path(
            args.val_sft_dataset_dir, task_name, "balanced_sft_dataset"
        )
    ...
    logger.info("=== Balanced multi-task backwards reasoning summary ===")
    ...
    logger.info("Balanced multi-task backwards reasoning pipeline completed.")
