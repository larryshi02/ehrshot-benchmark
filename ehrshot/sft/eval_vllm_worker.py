...
def construct_prompt(context: str, task_name: str) -> str:
    task_query = TASK_QUERIES[task_name]
    return (
        f"System: You may reason step by step, but your final output MUST be in the following format: "
        f"\"Final Answer: [Positive/Negative]<STOP>\".\n\n\n"
        f"User: You are a helpful medical assistant. Below is a patient's electronic healthcare record (EHR) in Markdown format. "
        f"Please answer the following query using clinically grounded reasoning: {task_query}. "
        f"Patient Medical History:\n\n\n\n{context}"
        f"\nIMPORTANT: You must conclude with your final prediction in the format "
        f"\"Final Answer: [Positive/Negative]<STOP>\""
    )
...
def main():
    ...
    for item in test_data:
        p = construct_prompt(item['context'], args.task_name)
        prompts.append(p)
        metadata.append({
