#!/usr/bin/env python3
"""
Fine-tune Qwen3-8B for Direct Clinical Outcome Prediction

Trains the model to predict "Positive" or "Negative" for clinical outcome tasks
(acute MI, hyperlipidemia, hypertension, pancreatic cancer) without reasoning.

Key Features:
    - Single-token classification output ("Positive" or "Negative")
    - Prompt masking: loss computed only on response tokens
...
Usage:
    python qwen_sft_direct.py \\
        ...
        --output_dir finetuned_models/originalEHR_train_all/acute_mi \\
        --wandb_project ehrshot-direct-originalEHR-train_all-qwen3-8b \\
    ...
    [
      {
        "conversations": [
          ...
          {"role": "user", "content": "Based on the patient's EHR..."},
...
Compatibility:
    - Trained models are evaluated by eval_vllm_direct.py
    - Prompt format must match construct_direct_yn_messages() in eval script
"""
